...
%plot acceleration and deceleration in colors
po2 = lhistory(:,[index.x_k2+1,index.y_k2+1]);
offset2 = 0.2*gokartforward(lhistory(:,index.theta_k2+1));
forward2 = gokartforward(lhistory(:,index.theta_k2+1));
p2 = offset2 + po2;
...
hold on
%yyaxis left
plot(lhistory(:,1), lhistory(:,index.ab_k2+1),'g')
plot(lhistory(:,1), lhistory(:,index.tv_k2+1),'b')
ylabel('acceleration [m/s^2]')
axis([-inf inf -1 3])
%yyaxis right
%plot(lhistory(:,1),lhistory(:,index.v+1))
...
beta2 = lhistory(:,index.beta_k2+1);
dotbeta2 = lhistory(:,index.dotbeta_k2+1);
tangentspeed2 = lhistory(:,index.v_k2+1);
...
fa2 = lhistory(:,index.ab_k2+1);
na2 = (fa2.^2+la2.^2).^0.5;
title('velocity')
axis([-inf inf -10 40])
ylabel('[km/h]')
xlabel('[s]')
plot(lhistory(:,1),lhistory(:,index.v_k2+1)*3.6);
plot(lhistory(:,1),lhistory(:,index.yv_k2+1)*3.6);
legend('v_x','v_y')
...
% 
% for ii=1:length(lhistory(:,index.v+1))
%     [ACCX2(ii),ACCY2(ii),ACCROTZ(ii)] = modelDx(lhistory(ii,index.v+1),lhistory(ii,index.yv+1),lhistory(ii,index.dottheta+1),ackermannAngle2(ii),lhistory(ii,index.ab+1),lhistory(ii,index.tv+1), param);
% end

% figure
% plot(lhistory(:,1),ACCX2,'b')
% hold on
% plot(lhistory(:,1),ACCY2,'r')
% plot(lhistory(:,1),zeros(length(ACCX2),1),'g')
% legend('AccX','AccY')
...
% plot(lhistory(:,1),(-lhistory(:,index.tv+1)/2+lhistory(:,index.ab+1))/0.73*1.19,'r')
% legend('+','-')
% vel2=zeros(2,length(lhistory(:,index.v+1)));

% for ii=1:length(lhistory(:,index.v+1))
%     vel2(:,ii) = (rotmat(lhistory(ii,index.beta+1))*[lhistory(ii,index.v+1);lhistory(ii,index.yv+1)+l1*lhistory(ii,index.dottheta+1)])';
%     f1y2(ii)= simplefaccy(vel2(2,ii),vel2(1,ii));
%     F12(:,ii) = rotmat(-lhistory(ii,index.beta+1))*[0;f1y2(ii)]*f1n;
% end

% figure
% plot(lhistory(:,1),F12(1,:)'+lhistory(:,index.ab+1),'r')
% hold on
% plot(lhistory(:,1),ACCX2,'b')