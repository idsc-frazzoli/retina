...
int ConfigIO_example(HANDLE hDevice, int *isDAC1Enabled)
{
    float binaryBlob_data[5];
    idsc_BinaryBlob binaryBlob;
    binaryBlob.data_length = 5 * 4;
    binaryBlob.data = (int8_t*)binaryBlob_data;
...
int StreamData_example(HANDLE hDevice, u3CalibrationInfo *caliInfo, int isDAC1Enabled)
{
    ...
    numReadsPerDisplay = 1; // original code: 24
     ...
     */
    double voltages[(SamplesPerPacket/NumChannels)*readSizeMultiplier*numReadsPerDisplay][NumChannels];
...
//printf("aomw\n");
        ...
        for( k = 0; k < NumChannels; k++ )
            binaryBlob_data[k]=voltages[0][k];
        // TODO publish
  //usleep(100000);
    }
