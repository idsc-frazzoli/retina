...
public class GokartRender extends AbstractGokartRender {
  private final VehicleModel vehicleModel = RimoSinusIonModel.standard();
  ...
  private final AxisAlignedBox aabLinmotPos = new AxisAlignedBox(RealScalar.of(0.15));
  // ---
  ...
  public void protected_render(GeometricLayer geometricLayer, Graphics2D graphics) {
    ...
    { // rear wheel odometry
      ...
      for (int wheel = 0; wheel < 2; ++wheel) {
        ...
        graphics.fill(geometricLayer.toPath2D(aabRimoRate.alongY(rateY_draw.Get(0 + wheel))));
    ...
    { // rear wheel torques
      ...
      for (int wheel = 0; wheel < 2; ++wheel) {
        Tensor matrix = Se2Utils.toSE2Translation(vehicleModel.wheel(2 + wheel).lever().add(ofs[wheel]));
        geometricLayer.pushMatrix(matrix);
        graphics.fill(geometricLayer.toPath2D(aabRimoRate.alongY(RealScalar.of(trq[0 + wheel]))));
        geometricLayer.popMatrix();
      }
    }
    { // brake
    ...
    if (gokartStatusEvent.isSteerColumnCalibrated()) {
      ...
      if (Objects.nonNull(lidarLocalizationModule)) {
        ...
        Tensor alongX = aabRimoRate.alongX(Magnitude.PER_SECOND.apply(angularSlip).negate());
