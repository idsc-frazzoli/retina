...
public class Se2LateralAcceleration implements CostFunction {
  private final Scalar weight;

  public Se2LateralAcceleration(Scalar weight) {
    this.weight = weight;
  }
  ...
  public Scalar costIncrement(GlcNode glcNode, List<StateTime> trajectory, Flow flow) {
    return cost(flow.getU(), StateTimeTrajectories.timeIncrement(glcNode, trajectory)).multiply(weight);
  }
