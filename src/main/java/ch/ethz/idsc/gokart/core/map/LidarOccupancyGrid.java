...
public class LidarOccupancyGrid implements RenderInterface, RayOccupancyGrid {
   ...
   * safety margin around detected obstacle pixels
   * @param angles of lidar rays in degree
   * @return BayesianOccupancyGrid with grid dimensions ceil'ed to fit a whole number of cells per dimension */
  public static LidarOccupancyGrid of(Tensor lbounds, Tensor range, Scalar cellDim, Scalar obstacleRadius, Integer[] angles) {
    return of(lbounds, range, cellDim, obstacleRadius, angles, false);
  }
   ...
   * @param angles of lidar rays in degree
   * @return BayesianOccupancyGrid with grid dimensions ceil'ed to fit a whole number of cells per dimension */
  public static LidarOccupancyGrid of(Tensor lbounds, Tensor range, Scalar cellDim, Scalar obstacleRadius, Integer[] angles, boolean fill) {
    ...
    return new LidarOccupancyGrid(lbounds, rangeCeil, dimension, obstacleRadius, angles, fill);
  }
...
//  private final double P_M_PASS = MappingConfig.GLOBAL.getP_M_PASS();
  // TODO not as originally intended
  private final double P_Z_HIT = MappingConfig.GLOBAL.getP_M_HIT(); // p(z=1|m=1)
  private final double P_Z_PASS = MappingConfig.GLOBAL.getP_M_PASS(); // p(z=1|m=0)
...
//  private final double[] PREDEFINED_P = { 1 - P_M_HIT, P_M_HIT, P_M_PASS, 1 - P_M_PASS };
  private final double[] PREDEFINED_P = { 1 - P_Z_HIT, P_Z_HIT, 1 - P_Z_PASS, P_Z_PASS }; // p(z=0|m=1), p(z=1|m=1), p(z=0|m=0), p(z=1|m=0)
  /** forgetting factor for previous classifications */
  private final double lambda = MappingConfig.GLOBAL.getLambda();
  private final double norm = 1 + lambda;
...
//  private double[] logOdds;
  /** array containing current probabilities of each cell */
  private double[] prob;
  ...
  private LidarOccupancyGrid(Tensor lbounds, Tensor rangeCeil, Dimension dimension, Scalar obstacleRadius, Integer[] angles, boolean fillMap) {
    ...
    */
    prob = new double[dimx * dimy];
    if (!fillMap)
      Arrays.fill(prob, P_M);
    else {
      Arrays.fill(prob, 0.99);
      setHset();
    }
    // ---
   ...
   * 
   * @param pos 3D position of new lidar observation in gokart coordinates
   * @param type of observation either 0, or 1 */
  @Override // from RayOccupanyGrid
    ...
    if (Objects.nonNull(gokart2world)) {
      ...
      if (0 <= pix && pix < dimx) {
        ...
        if (0 <= piy && piy < dimy) {
          ...
          */
          double pPrev = prob[idx];
          updateCellProb(pix, piy, type);
          double p = prob[idx];
          // Max likelihood estimation
          synchronized (hset) {
            // TODO why also check previous value?
            if (P_THRESH < p && pPrev <= P_THRESH)
              hset.add(cell);
            else //
              if (p < P_THRESH && P_THRESH <= pPrev)
                hset.remove(cell);
          }
        }
      }
  ...
  public void setNewlBound(Tensor lbounds) {
    if (Objects.nonNull(gokart2world)) {
      ...
      */
      ...
      double[] probNew = new double[dimx * dimy];
      Arrays.fill(probNew, P_M);
      synchronized (hset) {
        hset.clear();
        Tensor trans = lidarToCell(toPos(Tensors.vector(0, 0))); // calculate translation
        final int ofsx = trans.Get(0).number().intValue();
        ...
        for (int i = 0; i < dimx; i++)
          for (int j = 0; j < dimy; j++) {
            double p = prob[j * dimx + i];
            int pix = i + ofsx;
            if (0 <= pix && pix < dimx) {
              int piy = j + ofsy;
              if (0 <= piy && piy < dimy) {
                probNew[piy * dimx + pix] = p;
                if (p > P_THRESH)
                  hset.add(Tensors.vector(pix, piy));
              }
            }
  ...
  */
  ...
  /** Update the probability of a cell using the probability of occupation given a new observation.
   * p_t = p(m|z) = p(z|m) * p(m) / p(z)
   * @param pix of cell to be updated
   * @param piy of cell to be updated
   * @param type z: 0 = miss, 1 = hit */
  private void updateCellProb(int pix, int piy, int type) {
    int idx = piy * dimx + pix;
    double p_m = P_M;
    double p_z_m = PREDEFINED_P[type]; // PREDEFINED_P: p(z=0|m=1), p(z=1|m=1), p(z=0|m=0), p(z=1|m=0)
    double p_z = PREDEFINED_P[type] * p_m + PREDEFINED_P[type  + 2] * (1 - p_m);
    if (p_z == 0)
      throw new ArithmeticException("Overflow");
    double p_m_z = p_z_m * p_m / p_z;
    prob[idx] = (lambda * prob[idx] + p_m_z) / norm;
  }
