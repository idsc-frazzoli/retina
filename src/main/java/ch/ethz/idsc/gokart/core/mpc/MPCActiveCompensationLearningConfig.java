...
public class MPCActiveCompensationLearningConfig {
  ...
  public Scalar negativeAccelerationCorrectionRate = Quantity.of(0.1, SI.ACCELERATION.negate().multiply(RealScalar.of(2)).add(SI.PER_SECOND));
  /** how fast is the steering corrected scaled by steering rate (rotation per meter driven [m^-1]) */
  public Scalar steeringCorrectionRate = Quantity.of(0.1, SI.SECOND.add(SI.METER).add(SI.PER_SECOND));// ->gives us [m]
}
