    ...
    if (Scalars.lessThan(time_next, time)) {
      if (Objects.nonNull(rge) && Objects.nonNull(rpe)) {
        ...
        tensorBuilder.flatten( //
            ...
            rge.getTireL.getBusVoltage().map(RimoGetTire.MAGNITUDE_VOLT), //
            rge.getTireR.getBusVoltage().map(RimoGetTire.MAGNITUDE_VOLT), //
  ...
  public static void main(String[] args) throws IOException {
    for (DubendorfHangarLog dhl : DubendorfHangarLog.values()) {
      ...
      if (file.isFile()) {
        ...
        PowerRimoTracker rimoTracker = new PowerRimoTracker(Quantity.of(0.1, "s"));
