...
/* package */ class HandLabeler {
  ...
  static void drawEllipsesOnImage(Graphics2D graphics, List<ImageBlob> list) {
    list.forEach(imageBlob -> ImageBlobRender.of(graphics, imageBlob, Color.WHITE));
  }
  ...
  public HandLabeler(BlobTrackConfig pipelineConfig) {
    ...
    numberOfFiles = ImageEvaluationFolders.HANDLABEL.subfolder(imagePrefix).list().length;
  ...
  private void setBufferedImage() {
    ...
    File pathToFile = new File(ImageEvaluationFolders.HANDLABEL.subfolder(imagePrefix), fileName);
    try {
