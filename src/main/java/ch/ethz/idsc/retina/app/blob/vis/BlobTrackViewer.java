// code by mg
package ch.ethz.idsc.retina.app.blob.vis;
...
import ch.ethz.idsc.retina.app.blob.eval.ImageEvaluationFolders;
import ch.ethz.idsc.retina.app.filter.DavisDvsEventFilter;
...
public class BlobTrackViewer implements DavisDvsListener {
  ...
  public BlobTrackViewer(BlobTrackConfig blobTrackConfig, BlobTrackProvider blobTrackProvider) {
    ...
    parentFilePath = ImageEvaluationFolders.HANDLABEL.subfolder(logFilename);
  ...
  public void davisDvs(DavisDvsEvent davisDvsEvent) {
    ...
    if (saveFrame)
      if (timeStamp - lastSavingTimeStamp > savingInterval) {
        ++imageCount;
        SaveFrame.of(eventFrames[0].getAccumulatedEvents(), parentFilePath, logFilename, timeStamp, imageCount);
        lastSavingTimeStamp = timeStamp;
      }
  }
}
