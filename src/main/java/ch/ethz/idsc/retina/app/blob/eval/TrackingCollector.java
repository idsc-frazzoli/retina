...
import ch.ethz.idsc.retina.app.blob.algo.ImageBlobSelector;
 ...
 * 
 * if no features are hand-labeled at a certain instant, we do not collect the estimatedFeatures at that instant. */
public class TrackingCollector implements DavisDvsListener {
  ...
  public TrackingCollector(ImageBlobSelector imageBlobSelector, BlobTrackConfig pipelineConfig) {
    ...
    numberOfLabelInstants = ImageEvaluationFolders.HANDLABEL.subfolder(imagePrefix).list().length;
