...
public class MPCSimpleBrakingTest extends TestCase {
  public void testDerivative() {
    MPCBraking mpcBraking = new MPCSimpleBraking();
    // this has no effect
    ...
    mpcBraking.getControlAndPredictionSteps(cns);
    // System.out.println(braking.getBraking(Quantity.of(0.1, SI.SECOND)));
    // test
    assertTrue(Chop._05.close(//
        mpcBraking.getBraking(Quantity.of(0.1, SI.SECOND)), RealScalar.ZERO));
    assertTrue(Chop._05.close(//
        mpcBraking.getBraking(Quantity.of(1.1, SI.SECOND)), RealScalar.ZERO));
    // Scalar braking2 =
    // TODO MH this returns 0
    mpcBraking.getBraking(Quantity.of(2.1, SI.SECOND));
  ...
  public void testSimple() {
    MPCBraking mpcBraking = new MPCSimpleBraking();
    Scalar scalar = mpcBraking.getBraking(Quantity.of(0.3, SI.SECOND));
