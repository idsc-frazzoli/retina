...
public class GokartPoseEventsTest extends TestCase {
  ...
  public void testExtended() {
    ...
    assertEquals(gokartPoseEvent.getVelocity().extract(0, 2), Tensors.fromString("{4[m*s^-1], 5[m*s^-1]}"));
  ...
  public void testSimple() {
    GokartPoseEvent gokartPoseEvent = GokartPoseEvents.offlineV1( //
        Tensors.fromString("{1[m], 2[m], 3}"), RealScalar.ONE);
    assertEquals(gokartPoseEvent.getVelocity().extract(0, 2), Tensors.fromString("{0[m*s^-1], 0[m*s^-1]}"));
